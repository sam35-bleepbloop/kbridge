...
async function main() {
  ...
  // ── Task pricing rules ─────────────────────────────────────────────────────
  // Initial known one-off task categories at launch.
  // Admin can add/edit/remove via /admin/pricing-rules.
  // Tasks with no matching rule are charged the Pioneer bonus rate (2 tokens).
  const pricingRules = [
    {
  ...
  for (const rule of pricingRules) {
    await db.taskPricingRule.upsert({
      where:  { category: rule.category },
      update: { tokenCost: rule.tokenCost, description: rule.description },
      create: rule,
    });
  }
  console.log(`✓ Seeded ${pricingRules.length} task pricing rules`);
